...
model BUILT_IN_MOS2 {
  ...
  independent {
	...
	name=NEFf default=1.0 positive 
	print_test="neff != 1.0 || lambda == NA";
      double ucrit_cm "critical field mobility degradation"
	name=UCRit default=1e4
	print_test="ucrit_cm != 1e4 || uexp != NA";
      double uexp "critical field exponent in mob.deg."
	name=UEXp default=NA;
      double utra "transverse field coefficient (not used)"
	name=UTRa default=NA print_test=false;
      double delta "width effect on threshold voltage" 
	name=DELta default=0.0;
    }
    calculated_parameters {
      double nfs "" calculate="nfs_cm*ICM2M2";
      double ucrit "" calculate="ucrit_cm*ICM2M";
      bool calc_kp "" default=false;
      double alpha "" calculate="((nsub != NA)
	? (2. * P_EPS_SI) / (P_Q * nsub)
	: 0.)";
      double xd "coeffDepLayWidth" calculate="sqrt(alpha)";
      double xwb "" calculate="((nsub != NA)
	? xd * sqrt(pb)
	: .25e-6)";
      double vbp "" calculate="ucrit * P_EPS_SI / cox";
	...
	if (vto == NA) {
	  double phi_ms = (tpg == gtMETAL)
	    ? int(polarity) * (-.05 - (egap + int(polarity) * phi) / 2.)
	    : -(tpg * egap + phi) / 2.;
	  double vfb = phi_ms - int(polarity) * P_Q * nss / cox;
  ...
  size_dependent {
    calculated_parameters {
      double relxj "" calculate="((m->xj != NA && m->xj > 0)
	       ? .5 * m->xj / l_eff
	       : NA)";
      double eta_1 "" calculate="((cgate != 0)
	       ? M_PI_4 * P_EPS_SI * m->delta / cgate * l_eff
	       : 0.)";
      double eta "" calculate="eta_1 + 1.";
      double eta_2 "" calculate="eta / 2.";
    }
  }
  temperature_dependent {
    calculated_parameters {
      ...
      double phi_sqrt_phi "" calculate="phi * sqrt_phi";
	...
	(m->vto - m->gamma * sqrt(m->phi)
	 +.5*(m->egap-egap) + double(m->polarity)* .5 * (phi-m->phi)), m->phi))";
    }
    code_pre {
      double temp = c->temp_k(par_scope->params());
