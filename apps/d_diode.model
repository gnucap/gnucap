...
device BUILT_IN_DIODE {
  ...
  common {
    ...
    code_pre {
      double temp_k = c->temp_k(par_scope);
      tempratio = temp_k / (m->_tnom_c+P_CELSIUS0);
      vt = P_K_Q * temp_k * m->n_factor;
    }
  }
  eval Cj {
  ...
  eval Yj {
    ...
    p->_isat = c->is_adjusted * pow(c->tempratio, m->xti)
      * exp((m->eg/vt) *(c->tempratio-1));
    trace4("", c->tempratio, vt, oldregion, p->_isat);
	...
	y.f1 = p->_isat/c->vt;	    /* guess that the voltage should be 0   */
