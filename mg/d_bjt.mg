...
device BUILT_IN_BJT {
  ...
  circuit {
    ...
    fpoly_cap Ccs {s ic} state=qcs // qcs, cqcs
	 // omit="!(_n[n_s].n_())"; unreachable, removed
	 ;
  ...
  common {
    ...
    calculated_parameters {
      ...
      double _vt "" calculate="P_K_Q * c->temp_k(par_scope)";
    }
  }
  tr_eval {
    int foo=3;
...
model BUILT_IN_BJT {
  ...
  temperature_dependent {
    ...
    code_pre {
      ...
      double temp_k =
	((has_hard_value(c->temp_c)) ? (c->temp_c + P_CELSIUS0) : c->temp_k(par_scope->params()));
  ...
  tr_eval {
	...
	if (has_nz_value(m->irb)) {untested();//554
	...
	double arg2  = NA;
	double arg3  = NA;
	if (has_nz_value(m->xtf)) {untested();//579
...
bool DEV_BUILT_IN_BJT::do_tr()
{
  ...
  if(_sim->is_initial_step()) {	// initial guess
    if (c->off) {
      ...
      vbe = c->_vt * log(c->_vt / (M_SQRT2 * m->ibe));
    }
    vbc = 0.;
    ...
    int polarity = int(m->polarity);
    vbe = pnj_limit((polarity * volts_limited(n_(n_ib), n_(n_ie))),
		    vbe, t->vt, t->Vcrit);
    vbc = pnj_limit((polarity * volts_limited(n_(n_ib), n_(n_ic))),
		    vbc, t->vt, t->Vcrit);
    vbx = polarity * volts_limited(n_(n_b), n_(n_ic));
    vcs = polarity * volts_limited(n_(n_s), n_(n_ic));
  }
