...
model BUILT_IN_MOS8 {
  ...
  independent {
    override {
      ...
      int cmodel "CMODEL" print_test="cmodel!=0"
	calculate="((!cmodel)?0:cmodel)";
	...
	name=EF default=1.0;
      ...
      // version 3.3
      int acnqsMod "AC NQS model selector"
	name=ACNQSMOD default=0 quiet_max=1;
      double toxm "Gate oxide thickness used in extraction"
	name=TOXM default=tox;
      double lintnoi "lint offset for noise calculation"
	name=LINTNOI default=0.0;
  
      double ijth "Diode limiting current" name=IJTH default=0.1;
      double tpb "Temperature coefficient of pb" name=TPB default=0.0;
      double tcj "Temperature coefficient of cj" name=TCJ default=0.0;
      double tpbsw "Temperature coefficient of pbsw" name=TPBSW default=0.0;
      double tcjsw "Temperature coefficient of cjsw" name=TCJSW default=0.0;
      double tpbswg "Temperature coefficient of pbswg" name=TPBSWG default=0.0;
      double tcjswg "Temperature coefficient of cjswg" name=TCJSWG default=0.0;

      double Llc "Length reduction parameter for CV" name=LLC default=Ll;
      double Lwc "Length reduction parameter for CV" name=LWC default=Lw;
      double Lwlc "Length reduction parameter for CV" name=LWLC default=Lwl;
      double Wlc "Width reduction parameter for CV" name=WLC default=Wl;
      double Wwc "Width reduction parameter for CV" name=WWC default=Ww;
      double Wwlc "Width reduction parameter for CV" name=WWLC default=Wwl;

      // level 49 parameters, probably dummy for now
      int acm "area calculation method, ignored" name=ACM default=10;
    }
  ...
  size_dependent {
	...
	name=ACDE default=1.0 quiet_min=0.4 quiet_max=1.6;
      double moin 
	"Coefficient for gate-bias dependent surface potential, capmod=3"
	name=MOIN default=15.0 quiet_min=5.0 quiet_max=25.0;
      double noff "C-V turn-on/off parameter, capmod=1,2,3"
	name=NOFF default=1.0 quiet_min=0.1 quiet_max=4.0;
      double voffcv "C-V lateral-shift parameter, capmod=1,2,3"
	name=VOFFCV default=0.0 quiet_min=-0.5 quiet_max=0.5;
      double alpha1 "substrate current model parameter"
	name=ALPHA1 default=0.0;
    }
    calculated_parameters {
      double dl;
      double dlc;
  ...
  temperature_dependent {
    ...
    code_pre {
      temp = c->temp_k(par_scope->params());
	...
	if (s->vth0 == NA) {
	  ...
	  vfb = int(m->polarity) * s->vth0 - phi - k1 * sqrtPhi;
	}
      }else{
	vfb = s->vfb;
	trace1("", s->vfb);
      ...
      if (s->vth0 == NA) {
	vth0 = int(m->polarity) * (vfb + phi + k1 * sqrtPhi);
      }else{
	vth0 = s->vth0;
      }
	...
	{
	  double tmp3 = int(m->polarity) * vth0 - T2 - T3 + s->k3 * T4 + T5;
  ...
  tr_eval {
	...
	trace1("", T4);
      ...
      trace2("", T1, DIBL_Sft);
      double Vth = int(m->polarity) * t->vth0 + t->k1ox * sqrtPhis
